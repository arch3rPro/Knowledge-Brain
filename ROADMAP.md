...
真实 Minisign public key、签名 secrets、三平台正式构建、签名校验和、build provenance 和公开 Release 已由 v0.1.0 与 v0.1.1 验证。v0.1.2 开始同时发布三个裸可执行文件与三个完整归档，并以正式发布工作流验证新增的 Agent Skills 与 MCP Streamable HTTP。macOS notarization 和 Windows Authenticode 因没有对应证书而保持 future；一步安装入口仍未实现。
