...
`kb serve` 提供本地 HTTP 接口和 SSE 进度。API 路径不包含版本前缀；请求和响应内容携带 schema 版本。SSE 事件按 `operation_id` 归属，客户端断线重连不能导致操作重复执行。
...
服务默认只监听回环地址。除不包含私有信息的健康检查和能力发现外，回环与局域网 HTTP 请求都必须携带会话令牌。局域网监听必须显式开启，写入权限必须再次显式开启。令牌随机生成、只显示一次，服务端只保存哈希；令牌、TLS 私钥和本机绑定地址保存在本机状态，不进入 Vault 的可移植配置。

局域网模式要求 TLS。用户可以显式允许不安全 HTTP，但程序必须持续显示警告，且不能将其设为默认。HTTP 服务不得使用通配 CORS；WebUI 使用同源策略、Origin 检查和防跨站请求机制。
