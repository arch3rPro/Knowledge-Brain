...
Appending an event rewrites `events.json` through the existing atomic replacement path. A duplicate transition with the same kind and progress counters is ignored, so retrying recovery or a completed operation does not create unbounded duplicates. Logs are capped by the operation's bounded work plus lifecycle events. Existing operation directories without a log return a synthetic current-state event in memory and are not mutated by inspection.
