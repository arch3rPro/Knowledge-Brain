...
来源更新后，旧的精确 `kb-source://...?sha256=...` 仍指向已保存的历史对象，不会因出现新版本而失效。引用旧版本的 Wiki 文档仍保留当时的证据链；lint 将其报告为 `source_version_outdated` 警告，而不是缺失错误。不能机械地把旧哈希替换成当前哈希：应先核对新版本是否改变正文结论，再通过知识保存流程一起更新内容和引用。只有引用格式错误或对应历史版本不存在时才报告 `source_version_missing`。
