...
Before installing any generated path, persist operation-owned progress under the user state directory. If application stops after some created paths appear, the next invocation considers only paths named by that progress record and removes a file only when its hash still equals the plan's generated bytes. A changed file produces `vault_needs_recovery` with its exact path; an unrecorded path is treated as a stale plan and is never removed.
