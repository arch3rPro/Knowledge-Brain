...
touch "$dist/knowledge-brain-v1.2.3-x86_64-unknown-linux-gnu"
touch "$dist/knowledge-brain-v1.2.3-aarch64-apple-darwin"
touch "$dist/knowledge-brain-v1.2.3-x86_64-pc-windows-msvc.exe"
bash "$checksum_script" "$dist"
[[ $(wc -l < "$dist/SHA256SUMS" | tr -d ' ') == 6 ]] || fail "checksum list must contain three archives and three executables"
